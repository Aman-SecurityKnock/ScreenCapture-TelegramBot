...
# ===============================
$CredFile = "C:\Temp\cred.dat"

# Use global credentials if defined, otherwise use default placeholders
if ($global:PlainBotToken) {
    $PlainBotToken = $global:PlainBotToken
} else {
    $PlainBotToken = "add your telegram token"
}
if ($global:PlainChatID) {
    $PlainChatID = $global:PlainChatID
} else {
    $PlainChatID = "add your chat id"
}
...
if (-not (Test-Path $CredFile)) {
    ...
    # Encrypt credentials using DPAPI (machine/user-specific)
    $CredObject = @{
        BotToken = ($PlainBotToken | ConvertTo-SecureString -AsPlainText -Force | ConvertFrom-SecureString)
        ChatID   = ($PlainChatID   | ConvertTo-SecureString -AsPlainText -Force | ConvertFrom-SecureString)
    }
